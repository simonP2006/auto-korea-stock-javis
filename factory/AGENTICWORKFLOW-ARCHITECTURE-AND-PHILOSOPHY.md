...
**검증 방법론 메타 교훈 — "추정은 검증이 아니다" (워크된 사례):** pACS는 *자기 추정*이고, 자기 추정은 그 자체로 결론이 될 수 없다. 한 빌드 세션에서 이 원리가 세 가지 구체적 교훈으로 확인되었다(상속 가능한 DNA):
- **추정 ≠ 검증.** 품질 점수를 기록하기 전에 반드시 *적대적 재채점*으로 닫는다(L2가 L1.5 자기 추정을 닫는 구조). 거장 5인×15기준 재채점이 자기 추정만으로는 드러나지 않던 회귀를 찾아냈다.
- **산문이 아닌 실행 가능한 단계로 행위를 검증한다.** 스펙 산문은 끝내 구현되지 않은 기계(phantom)를 묘사할 수 있다 — 한 단계는 미구현 cross-write를 사실인 양 서술하고 있었다. 행위는 서술이 아닌 실제 실행 경로에 대조해 검증해야 한다.
- **행위 변경 전 다중 에이전트 적대적 영향분석.** 다관점 영향분석이 phantom을 적발하고 오케스트레이터 자신의 선행 단독 분석 5건의 오류를 교정했다(절대 기준 3 — CCP Step 2의 다관점 적용). 검증의 *방향성*도 양방향이어야 한다 — 단방향 점검(예: CLAUDE.md→skill)은 실재하는 drift를 가린다.

근거 ADR: 빌드 DECISION-LOG ADR-056.
