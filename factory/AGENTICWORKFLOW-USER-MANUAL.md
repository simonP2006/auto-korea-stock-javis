...
> **검증 방법론 (ADR-056)**: 품질 점수(pACS 등)는 *자기 추정*이므로 그 자체로 결론이 될 수 없습니다 — 기록 전 적대적 재채점(L2)으로 닫습니다. 행위는 서술적 산문이 아니라 *실행 가능한 단계*에 대조해 검증하고(사양 산문은 구현되지 않은 phantom을 묘사할 수 있음), 문서-코드 정합성은 *양방향*으로 점검하며, 동작 변경 전 다중 에이전트 적대적 영향분석을 거칩니다. 상세: 루트 `DECISION-LOG.md` ADR-056.
