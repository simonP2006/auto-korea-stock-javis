...
| 워크플로우 실행 (workflow-executor) | `workflow-generator`가 만든 `workflow.md`를 실제로 실행하는 런타임 오케스트레이터 스킬. Gemini에서는 단일 세션이 `state.yaml`(SOT)을 먼저 읽어 재개 지점을 판단하고 단계별 역할을 순차 수행. SOT 단독 쓰기 + F-1~F-14 폴백 + Step 12 사람 검수 강제. `AGENTS.md §6, §10` 참조 |
