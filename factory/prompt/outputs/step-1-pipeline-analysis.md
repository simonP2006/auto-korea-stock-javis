# Step 1 — Pipeline Dependency Graph & Output Schema
...
**Call chain** — `run_full_flow()` 단일 함수에 모든 단계 위임. 내부 위임 트리:
...
**Call chain** (`run_full_research_flow` 의 ①·②·③ + Stage 0 까지만, 필터 단계 없음):
...
### Independent invocability per filter

7개 필터 모듈 모두 `__main__` 블록 보유. 모두 `python -m src.kiwoom.itemFilter.<모듈>` 형태로 단독 실행 가능.

| Filter Module | Has `__main__`? | Standalone CLI args | Notes |
|---|---|---|---|
| `chart60_120Filter.py` | Yes (line 1071) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` (인자 없으면 `--all`) | argparse 미사용. 단일 종목/일괄 모드 지원. |
| `chart240Filter.py` | Yes (line 686) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 동일 패턴. |
| `chartDayPreFilter.py` | Yes (line 522) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 동일 패턴. |
| `chartDayFilter.py` | Yes (line 909) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 동일 패턴. |
| `investorFilter.py` | Yes (line 695) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 동일 패턴. |
| `financeFilter.py` | Yes (line 543) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 동일 패턴. |
| `chart60Filter.py` | Yes (line 819) | `<stock> [YYYYMMDD]` 또는 `--all [YYYYMMDD]` | 보조 모듈 — 파이프라인엔 미포함 (chart60_120 으로 통합됨). |
| `Filter_condition_update.py` | Yes (line 299) | `[YYYYMMDD ...]` (다중 일자 순차 실행) | `python -m src.kiwoom.itemFilter.Filter_condition_update 20260518 20260519 20260520` |
| `stageMasterFilter.py` | Yes (line 713) | argparse 서브커맨드: `bootstrap` / `daily-update` / `validate` / `filter <YYYYMMDD>` / `show` | 7개 모듈 중 유일하게 argparse 사용. |

---

## (b) Output Format Catalog
...
**Source**: `save_stage_passed(results, stage_idx=1)` (`plain_text.py:123-164`), 내부 `render_stage_passed()` 의 `"\n".join(r.candidate.stk_nm for r in passed) + "\n"` (라인 120).

**Sample** (verbatim, `/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage1_chart60_120_passed.md`):
```
이브이첨단소재
코칩
유성티엔에스
...
**Type pattern (A/B/C/D/E) included?** **No.**
...
**Source**: `save_stage_passed(results, stage_idx=2)`, 동일 포맷.

**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage2_chart240_passed.md`):
```
이브이첨단소재
코칩
유성티엔에스
...
**Source**: `save_stage_passed(results, stage_idx=3)` (슬롯 3 = chartDayPre — `_STAGE_FILENAMES` 매핑 참조), 동일 포맷.

**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage2_1_chartDayPre_passed.md`):
```
LG에너지솔루션
더블유씨피
삼성전기
...
**Source**: `save_stage_passed(results, stage_idx=4)`, 동일 포맷.

**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage3_chartDay_passed.md`):
```
캐프
화신정공
아바텍
...
**Source**: `save_stage_passed(results, stage_idx=5)`, 동일 포맷.

**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage4_investor_passed.md`):
```
캐프
화신정공
국보디자인
...
**Source**: `save_stage_passed(results, stage_idx=6)`, 동일 포맷.

**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/stage5_finance_passed.md`):
```
캐프
화신정공
국보디자인
...
### `researchedCompany.md` (and variants)

#### `researchedCompany.md` — canonical

**Source**: `save_researched_company(results)` (`plain_text.py:68-97`).
**필터**: `final_selected=True` 인 종목만 (`render_plain_text` 라인 62).
**Sample** (`/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/researchedCompany.md`):
```
캐프
화신정공
국보디자인
...
#### `researchedCompany.p1.md` — **legacy / orphan**
...
#### `researchedCompany.p2.md` — **legacy / orphan**
...
#### Recommended canonical file for SHOW_RESULTS chain
...
| stamp (header) | `datetime.now().strftime("%Y-%m-%d %H:%M:%S")` (라인 202) | `2026-05-28 21:01:34` | 블록 단위. |
...
### Gap value inclusion: **Partial (텍스트 내장만, 구조화 필드 없음)**

**Evidence** — `Filter_condition_update.py:152-157`:
```python
if selected:
    return (
...
### Example log lines (verbatim from `/Users/tajun/spJavis/kiwoom-rest-trader/reports/20260528/masterReference.log`)
...
- [x] Sub-task (a): 3 scripts traced (`run_full_research_flow.py`, `run_prefetch.py`, `run_filters.py`) + 9개 필터 모듈 (`chart60_120Filter`, `chart240Filter`, `chartDayPreFilter`, `chartDayFilter`, `investorFilter`, `financeFilter`, `chart60Filter`, `Filter_condition_update`, `stageMasterFilter`) `__main__` 블록 모두 라인 번호와 함께 명시.
