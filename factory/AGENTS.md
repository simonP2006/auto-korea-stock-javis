...
| Sub-agent | `Agent` 도구로 생성 | **읽기 전용** | Orchestrator가 호출 시 |
| Teammate | `Agent` + `TeamCreate`로 생성 | **읽기 전용** | Team Lead가 할당 시 |

#### Sub-agent 호출 프로토콜

Orchestrator가 Sub-agent(`@translator`, `@reviewer`, `@fact-checker`)를 호출할 때의 표준 프로토콜:

**1. 호출 방법**: `Agent` 도구의 `subagent_type` 파라미터로 에이전트 이름 지정
```
...
**3. 결과 수신**: Sub-agent 종료 시 `Agent` 도구가 결과를 반환한다.
...
Team Lead(=Orchestrator)
  ...
  3. Agent(subagent_type, team_name, ...) → Teammate 생성
...
├── DECISION-LOG.md          ← 프레임워크 설계 결정 로그 (ADR 51개; 제품 ADR은 docs/architectural-decision-records.md에 별도)
├── COPYRIGHT.md          ← 저작권
├── soul.md               ← DNA 유전 정의
├── docs/                 ← protocols/ 5개 + 제품 빌드 기준 문서 3개 (code-convention.md, code-quality-guide.md, architectural-decision-records.md[제품 ADR 14개·별도 네임스페이스])
├── .claude/
│   ├── settings.json          ← Hook 설정 (7개 이벤트: Stop·PostToolUse[5그룹]·PreCompact·SessionStart·PreToolUse[3]·SessionEnd·Setup[2] + permissions.deny Layer 0)
│   ├── agents/                ← Sub-agent 16개
│   │   │   ── 제네릭 게놈 에이전트 3 (name/description frontmatter) ──
│   │   ├── translator.md     (영→한 번역 — glossary 기반 용어 일관성)
│   │   ├── reviewer.md       (Adversarial Review — 코드/산출물 비판적 분석, 읽기 전용)
│   │   ├── fact-checker.md   (Adversarial Review — 외부 사실 검증, 웹 접근)
│   │   │   ── stock-filter 빌드 전용 13 (minimal frontmatter: model+tools+maxTurns) ──
│   │   ├── param-extractor.md · pipeline-analyzer.md · error-analyzer.md(sonnet)  (Step 1 리서치 팀)
│   │   ├── research-integrator.md (Step 2) · architect.md (Step 4) · claude-md-designer.md (Step 5)
│   │   ├── scan-designer.md · tune-designer.md (Step 6 팀) · claude-md-builder.md (Step 8)
│   │   └── scan-builder.md · tune-builder.md (Step 9 순차) · infra-validator.md (Step 10) · smoke-tester.md (Step 11)
│   ├── commands/              ← Slash Command 10개
│   │   ├── install.md         (/install — Setup Init 검증 결과 분석)
│   │   ├── maintenance.md     (/maintenance — Setup Maintenance 건강 검진)
│   │   ├── review-research.md (/review-research — Step 3 리서치 승인 게이트, 승인 시 current_step:4)
│   │   ├── review-design.md   (/review-design — Step 7 설계 승인 게이트, 승인 시 current_step:8)
│   │   ├── review-translation.md (/review-translation — 번역 현황 대시보드)
│   │   ├── accept-system.md   (/accept-system — Step 12 최종 검수, 항상 사람 검증·Autopilot 자동승인 예외)
│   │   ├── setup-prompts.md   (/setup-prompts — 프롬프트 placeholder 치환)
│   │   ├── verify-prompts.md  (/verify-prompts — 프롬프트 무결성 검증)
│   │   ├── run-prompts.md     (/run-prompts — 프롬프트 러너 시작)
│   │   └── resume-prompts.md  (/resume-prompts — 중단된 러너 재개)
│   ├── hooks/scripts/         ← Context Preservation System + Setup Hooks + Safety Hooks
│   │   ├── context_guard.py   (Hook 통합 디스패처 — 4개 이벤트의 단일 진입점)
│   │   ├── _context_lib.py    (공유 라이브러리 — 파싱, 생성, SOT 캡처, Smart Throttling, Autopilot 상태 읽기·검증, ULW 감지·준수 검증, 절삭 상수 중앙화, sot_paths() 경로 통합, 다단계 전환 감지, 결정 품질 태그 정렬, Error Taxonomy 12패턴+Resolution 매칭, Success Patterns(Edit/Write→Bash 성공 시퀀스 추출), IMMORTAL-aware 압축+감사 추적, E5 Guard 중앙화(is_rich_snapshot+update_latest_with_guard), Knowledge Archive 통합(archive_and_index_session — 부분 실패 격리), 경로 태그 추출(extract_path_tags), KI 스키마 검증(_validate_session_facts — RLM 필수 키 보장), SOT 스키마 검증(validate_sot_schema — 워크플로우 state.yaml 구조 무결성 8항목 검증: S1-S6 기본 + S7 pacs 5필드(dimensions, current_step_score, weak_dimension, history, pre_mortem_flag) + S8 active_team 5필드(name, status(partial|all_completed), tasks_completed, tasks_pending, completed_summaries)), Adversarial Review P1 검증(validate_review_output R1-R5, parse_review_verdict, calculate_pacs_delta, validate_review_sequence), Translation P1 검증(validate_translation_output T1-T7, check_glossary_freshness T8, verify_pacs_arithmetic T9 범용, validate_verification_log V1a-V1c), Predictive Debugging P1(aggregate_risk_scores+validate_risk_scores RS1-RS6+_RISK_WEIGHTS 13개 가중치+_RECENCY_DECAY_DAYS 감쇠), pACS P1 검증(validate_pacs_output PA1-PA7 — pACS 로그 구조 무결성: 파일 존재·최소 크기·차원 점수·Pre-mortem·min() 산술·Color Zone·RED 임계값 차단), L0 Anti-Skip Guard(validate_step_output L0a-L0c — 산출물 파일 존재+최소 크기+비공백), Team Summaries KI 아카이브(_extract_team_summaries — SOT active_team.completed_summaries → KI 보존), Abductive Diagnosis Layer(diagnose_failure_context 사전 증거 수집 + validate_diagnosis_log AD1-AD10 사후 검증 + _extract_diagnosis_patterns KA 아카이빙 + Fast-Path FP1-FP3 + 가설 우선순위 H1/H2/H3/H4), 모듈 레벨 regex 컴파일(9개+8개+8개+4개+5개 패턴 — 프로세스당 1회))
...
│   │   ├── validate_state_yaml.py (PostToolUse SOT 무결성 가드 — prompt/.claude/state.yaml 스키마 검증 W-1~W-4, exit 2 차단, infra_schema.py enum import — 유일한 차단형 PostToolUse 훅)
│   │   ├── monitor_translation_output.py (PostToolUse 번역 산출물 모니터 — .ko.md T-1~T-3 spot-check, 항상 exit 0)
...
│       ├── workflow-executor/    ← 워크플로우 실행 오케스트레이터 (runtime executor)
│       │   ├── SKILL.md          (prompt/workflow.md 12단계 구동, SOT 단독 쓰기, F-1~F-14 폴백, Step 9 순차, Step 12 항상 human-block; frontmatter 없음)
│       │   └── references/       (step-dispatch.md, fallback-paths.md)
...
│   ├── prd-research/             (4라운드 심층조사 — raw 20 + synthesis 16)
│   ├── .claude/state.yaml        (SOT — current_step:12, status:completed_degraded)
│   ├── .claude/codegen/          (6모듈 값-동등성 파이프라인 — infra_schema/generate_infra/validate_infra/param_ast/validate_param_values/unit_conversion)
│   ├── .claude/tests/            (pytest L1 — 55 테스트) · .claude/step-registry.yaml
│   ├── outputs/                  (12단계 산출물 EN 10 + KO 10 + pacs-logs/)
│   └── crystalize-prompt.md · distill-partner.md · crawling-skill-sample.md (제네릭 프롬프트 헬퍼)
├── prompt-runner/       ← 무인 실행 하니스 (run.py 1680줄, 110 프롬프트/35 세션, 실행 완료)
...
`workflow-generator`가 생성한 `workflow.md`를 **실제로 실행**하는 런타임 오케스트레이터 스킬. 설계자(designer)에 대응하는 실행자(executor) 짝이며, "워크플로우에 기술된 내용이 실제로 동작하는 것"(최종 목표)을 담당한다.

- **트리거**: 워크플로우 실행 세션 시작/재개. 세션 시작 시 항상 `prompt/.claude/state.yaml`(SOT)을 먼저 읽어 재개 지점을 판단
- **진입점**: `.claude/skills/workflow-executor/SKILL.md` (+ `references/step-dispatch.md`, `references/fallback-paths.md`). frontmatter가 없으며, 레지스트리 설명은 H1 제목("Workflow Executor — Stock Filter Orchestration Build")에서 파생
- **핵심 책임**:
  1. **SOT 단독 작성자** — 진행 상태를 `state.yaml`에만 기록(절대 기준 2). `validate_state_yaml.py` Hook이 무효 쓰기를 exit 2로 차단
  2. **단계별 에이전트 디스패치** — 16개 서브에이전트를 Step에 매핑 (Step 1·6 팀 병렬, Step 9 순차)
  3. **품질 게이트 집행** — 4계층(L0→L1→L1.5→L2) + pytest L1 + 번역 디스패치(`@translator`)
  4. **F-1~F-14 폴백 분류** — 실패 시 `completed_degraded` 상태 + `degradation_notes` 기록 후 진행
  5. **Step 12 hard-block** — Autopilot 활성 여부와 무관하게 항상 사람 검수(`/accept-system`)를 요구
...
| **Gemini CLI** | `GEMINI.md` (+ `.gemini/settings.json`) | Yes | 설정으로 추가 로드 |
...
| 영역 | 위치 | 내용 |
|------|------|------|
| **실행 스펙** | `prompt/workflow.md` (855줄) | 실제 실행된 12단계 워크플로우. `workflow-coding.md`(2143줄)는 구현 청사진, `prd.md`(776줄)는 요구사항, `prd-research/`는 4라운드 심층조사(raw 20 + synthesis 16) |
| **SOT** | `prompt/.claude/state.yaml` | 단일 진행 상태 — `current_step: 12`, `status: completed_degraded`, `translation_tasks`, `outputs`(EN+KO 쌍), `corrections`, `autopilot.decisions`(pACS 내장) |
| **실행자** | `.claude/skills/workflow-executor/` | 위 스펙을 구동하는 런타임 오케스트레이터(§6) — SOT 단독 쓰기, 16-에이전트 디스패치, 4계층 게이트, F-1~F-14 폴백, Step 12 hard-block |
| **에이전트 함대** | `.claude/agents/` 중 13개 | Step별 매핑: Step 1 리서치 팀(param-extractor·pipeline-analyzer·error-analyzer[sonnet]) → Step 2 통합(research-integrator) → Step 4 아키텍처(architect) → Step 5 설계(claude-md-designer) → Step 6 팀(scan-designer·tune-designer) → Step 8·9 빌드(claude-md-builder, scan-builder·tune-builder 순차) → Step 10 검증(infra-validator) → Step 11 스모크(smoke-tester) |
| **게이트 커맨드** | `.claude/commands/` 중 4개 | `/review-research`(Step 3), `/review-design`(Step 7), `/accept-system`(Step 12·항상 사람), `/review-translation`(번역 대시보드) |

### 값-동등성 코드젠 파이프라인 (P1 유전자의 극한 구현)

`prompt/.claude/codegen/`은 "AI가 값을 만들지 않고 코드 AST가 originate한다"는 P1 원칙(ADR-024/037/039 계보)을 빌드 레벨에서 물질화한 6모듈 계층이다.

- `infra_schema.py` — 빌드 그래프 **단일 SOT** (13-에이전트 `AGENT_ROSTER` + 12-엔트리 `STEP_DISPATCH` + enum + 7 `FILTER_MODULES`)
- `generate_infra.py` (G-1~G-7) — state.yaml/agent frontmatter/step-dispatch.md/conftest를 결정론적으로 생성
- `validate_infra.py` (V-1~V-10) — 교차 참조 게이트
- `param_ast.py` — 배포 코드에서 87개 Final 상수(32개 수치) 추출하는 무할루시네이션 오라클
- `validate_param_values.py` — 42개 하드 값-동등성 검사 ("code doesn't lie")
- `unit_conversion.py` — 라운드트립 오라클

### 검증 계층

- **제품 pytest L1**: `tests/`(최상위 3파일 64 테스트 — codegen 17 + SOT 스키마 21 + 상태머신 불변식 26) + `prompt/.claude/tests/`(10파일 + conftest + runner, 55 테스트) = **119 테스트** (단계 03/07/12 커버리지 공백 존재)
- **프레임워크 Safety Hook 테스트**: 별도 131개 (output_secret_filter 44 / sensitive_file_guard 44 / block_destructive 43)
- **무인 실행 하니스**: `prompt-runner/run.py`(1680줄)가 110 프롬프트/35 세션을 자동 구동 — 2026-05-26 완료(75 실행 + 35 clear, 0 실패; 21건 "context window" suspicious는 모두 오탐)

### 제품 전용 문서 및 ADR 네임스페이스

`docs/` 최상위에 빌드 기준 문서 3종이 있다: `code-convention.md`(명명·마크다운·8개 내용 규칙), `code-quality-guide.md`(5개 가중 품질 차원 30/25/20/15/10 + TS-1~5 안전 규칙), `architectural-decision-records.md`. **마지막 파일은 제품 전용 ADR 로그(ADR-001~014)로, 루트 `DECISION-LOG.md`(프레임워크 ADR-001~051)와 번호 체계가 완전히 분리**되어 있다. 두 네임스페이스를 혼동하지 말 것. 빌드 자체의 설계 결정은 루트 `DECISION-LOG.md` ADR-052~056에 기록되었다. **빌드 이후 제품 진화**: ADR-013(자연어 "시작" → 제품 사용 모드 진입 + Infrastructure Build 노출 차단), ADR-014(거장 5인×15기준 재채점 회귀 수정 — tuning-log를 filter-tune 단일 쓰기자로 확정한 Option B + screener_state.last_results_summary backfill + found-but-pending 가드, 가드레일 테스트 22개는 제품 저장소 `kiwoom-rest-trader/tests/orchestration/test_tuning_log_single_writer.py`에 존재).

### 현재 상태 (2026-05-31)

빌드는 Step 11까지 기능적으로 완료(`status: completed_degraded`)되었으나, **Step 12 사람 최종 검수는 Autopilot 자동승인으로 대체되어 실제 수행되지 않았다.** `pacs-logs/`에는 번역 pACS 7건만 있고 `autopilot-logs/`·`verification-logs/`·`review-logs/`는 비어 있어, L1 검증·L2 리뷰 증거 추적이 불완전하다. 재스캔 전까지 캐시된 리포트는 stale 상태다.
