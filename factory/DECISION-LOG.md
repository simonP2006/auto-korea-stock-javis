...
> **이하 ADR-052~056는 프레임워크를 사용해 제품(kiwoom-rest-trader stock-filter 스크리너)을 빌드하면서 내린 설계 결정이다. 제품 자체의 내부 ADR은 별도 네임스페이스(`docs/architectural-decision-records.md`, ADR-001~014)에 있으며 본 로그와 번호 체계가 다르다.**

### ADR-052: workflow-executor 스킬 + 13개 빌드 에이전트 + 4개 게이트 커맨드 — designer의 런타임 executor

- **날짜**: 2026-05-26
- **상태**: Accepted
- **맥락**: `workflow-generator`는 `workflow.md`(설계도)를 생성하지만, 그것을 **실제로 실행**하는 결정론적 주체가 없었다. 최종 목표("워크플로우에 기술된 내용이 실제로 동작")를 달성하려면 단계 디스패치·SOT 갱신·품질 게이트·폴백을 일관되게 수행하는 실행 계층이 필요. 키움 REST 주식 스크리너 빌드(12단계)를 첫 대상으로 실행 계층을 구현.
- **결정**:
  1. `workflow-executor` 스킬 신설 — designer(`workflow-generator`)에 대응하는 runtime executor. `prompt/workflow.md`의 12단계를 단일 진입점에서 구동하며, 세션 시작 시 항상 `prompt/.claude/state.yaml`(SOT)을 먼저 읽어 재개 지점을 판단. frontmatter 없이 H1 제목에서 레지스트리 설명 파생.
  2. 빌드 전용 13-에이전트 로스터 추가 (Step 1 리서치 팀 3 → Step 2 통합 1 → Step 4 아키텍트 1 → Step 5/6 설계 3 → Step 8/9 빌드 3 → Step 10/11 검증 2). 일회성 워크플로우 전용이라 제네릭 3-에이전트(translator/reviewer/fact-checker)와 frontmatter 규약이 다름(name/description 없음, model+tools+maxTurns). `error-analyzer`만 sonnet, 나머지 opus.
  3. 4개 게이트 커맨드 추가 — `/review-research`(Step 3), `/review-design`(Step 7), `/review-translation`(번역 대시보드), `/accept-system`(Step 12, 항상 사람 검증).
- **근거**:
  - **최종 목표 직접 구현**: 설계(generator)와 실행(executor)의 분리는 ADR-001("동작하는 시스템이 최종 산출물")의 자연스러운 귀결.
  - **절대 기준 2 보존**: executor를 SOLE SOT writer로 두어 병렬 에이전트의 SOT 동시 쓰기를 원천 차단.
  - **ADR-019 예외 명문화**: Step 12를 Autopilot과 무관하게 hard-block하여 "자동화할 수 없는 최종 인수"를 보장.
  - 13-agent는 ADR-021(Agent Team)/ADR-034(Adversarial Review) 패턴 재사용.
- **대안**: (1) generator가 실행까지 담당 → 기각(설계/실행 관심사 혼재, SRP 위반). (2) Autopilot이 Step 12까지 자동승인 → 기각(최종 인수는 사람의 판단 영역).
- **관련 파일**: `.claude/skills/workflow-executor/{SKILL.md, references/step-dispatch.md, references/fallback-paths.md}`, `.claude/agents/`(13 신규), `.claude/commands/{review-research, review-design, review-translation, accept-system}.md`, `prompt/workflow.md`, `prompt/.claude/state.yaml`

### ADR-053: 코드젠 결정론적 값-동등성 파이프라인 + pytest L1 계층 — "code doesn't lie"

- **날짜**: 2026-05-26
- **상태**: Accepted
- **맥락**: LLM이 빌드 산출물의 수치 파라미터·인프라 구조를 직접 생성하면 할루시네이션 표면이 커진다. P1 원칙(ADR-024/037/039)을 빌드 레벨로 확장하여 "코드 AST가 값을 originate하고 LLM은 originate하지 않는다"를 강제할 필요.
- **결정**:
  1. `prompt/.claude/codegen/` 6모듈 파이프라인 신설: `infra_schema.py`(빌드 그래프 단일 SOT — 13-에이전트 `AGENT_ROSTER` + 12-엔트리 `STEP_DISPATCH` + enum + 7 `FILTER_MODULES`) → `generate_infra.py`(G-1~G-7 결정론적 생성: state.yaml/agent frontmatter/step-dispatch.md/conftest) → `validate_infra.py`(V-1~V-10 교차 참조 게이트).
  2. 값-동등성 오라클: `param_ast.py`가 배포 코드에서 87개 Final 상수(32개 수치)를 AST로 추출, `validate_param_values.py`가 42개 하드 값-동등성 검사, `unit_conversion.py`가 라운드트립 검증.
  3. pytest L1 계층: `tests/`(최상위 3파일 64 — codegen 17 + SOT 스키마 21 + 상태머신 불변식 26) + `prompt/.claude/tests/`(10파일 + conftest + runner, 55) = **119 테스트** (프레임워크 Safety Hook 131 테스트와 별도).
- **근거**: P1의 극단적 적용 — 반복적으로 100% 정확해야 하는 값 검증을 결정론적 코드로 강제하여 LLM 할루시네이션 표면을 0으로. (제품 ADR `docs/architectural-decision-records.md` ADR-004 "Python Final constants as SOT"와 상호보완.)
- **대안**: (1) LLM이 값 검증 → 기각(비결정론적, 절대 기준 1 위반). (2) 스키마만 검증하고 값은 미검증 → 기각(doc-code drift 재발).
- **관련 파일**: `prompt/.claude/codegen/*.py`(6), `tests/*.py`(3), `prompt/.claude/tests/`(12)

### ADR-054: SOT 무결성 차단 훅(validate_state_yaml.py) + 번역 출력 모니터(monitor_translation_output.py)

- **날짜**: 2026-05-31
- **상태**: Accepted
- **맥락**: 빌드 중 SOT(`state.yaml`)가 잘못된 status/current_step으로 덮어써지는 사고가 실제 발생(2026-05-31). 사후 검증(orchestrator-invoked validators)만으로는 무효 쓰기를 사전 차단하지 못함. 또한 `.ko.md` 번역 산출물의 즉시 품질 신호가 없었음.
- **결정**:
  1. `validate_state_yaml.py` 신설 — PostToolUse(Write\|Edit) 훅으로 `prompt/.claude/state.yaml` 쓰기를 W-1~W-4 스키마로 검증(workflow.status enum, current_step 1-12 정수, translation_tasks status enum + attempt≥0, output 키 `^step-\d+-` 패턴). 위반 시 **exit 2로 차단** — 전체 PostToolUse 훅 중 유일한 차단형. enum은 `prompt/.claude/codegen/infra_schema.py`에서 import(`_context_lib`와 의도적으로 분리된 스키마).
  2. `monitor_translation_output.py` 신설 — PostToolUse(Write) 훅으로 `.ko.md` 쓰기를 T-1(크기)/T-2(glossary spot-check)/T-3(미번역 블록) 점검. 비차단(항상 exit 0), full P1은 orchestrator의 `validate_translation.py`에 위임.
- **근거**: 절대 기준 2(SOT 무결성)를 결정론적으로 사전 강제. 번역 모니터는 차단 시 워크플로우 방해 위험이 있어 informational(exit 0)로 한정 — ADR-036(경고-전용 패턴) 재사용. PostToolUse 그룹이 3 → 5로 확대, 스크립트 수 27 → 29.
- **대안**: (1) 사후 validator만 유지 → 기각(clobber 사고 재발). (2) 번역 모니터도 차단형 → 기각(과도 차단, 진행 방해).
- **관련 파일**: `.claude/hooks/scripts/validate_state_yaml.py`(신규), `.claude/hooks/scripts/monitor_translation_output.py`(신규), `.claude/settings.json`, `prompt/.claude/codegen/infra_schema.py`, `tests/test_sot_schema_enforcement.py`

### ADR-055: 제품 빌드 기준 문서 3종 신설 + 별도 제품 ADR 네임스페이스

- **날짜**: 2026-05-30
- **상태**: Accepted
- **맥락**: 제품 빌드의 코딩 규약·품질 기준·아키텍처 결정은 프레임워크 기준과 분리되어야 한다(ADR-010 "문서 분리" 정신). 빌드 산출물을 객관적으로 검증할 기준 문서가 필요.
- **결정**:
  1. `architect` 에이전트가 `docs/` 최상위에 3종 신설: `code-convention.md`(명명·마크다운 경계·8개 내용 규칙·검증 명령), `code-quality-guide.md`(5개 가중 품질 차원 30/25/20/15/10 + 체크리스트 + TS-1~5 안전 규칙), `architectural-decision-records.md`.
  2. `architectural-decision-records.md`는 **제품 전용 ADR 로그(ADR-001~014)**로, 루트 `DECISION-LOG.md`(프레임워크 ADR-001~051)와 번호 체계를 분리. `infra-validator`가 이를 소비.
- **근거**: 제품/프레임워크 관심사 분리(ADR-010). 명시 플래그: ADR 네임스페이스가 둘(프레임워크 51 + 제품 14)이 되어 혼동 위험이 있으므로 본 ADR로 명문화하고 루트 로그의 "위치" 항목에 cross-reference 추가.
- **대안**: (1) 제품 ADR을 루트 DECISION-LOG에 통합 → 기각(번호 충돌, 관심사 혼재). (2) 기준 문서 없이 빌드 → 기각(검증 기준 부재, 품질 측정 불가).
- **관련 파일**: `docs/code-convention.md`(신규), `docs/code-quality-guide.md`(신규), `docs/architectural-decision-records.md`(신규), `DECISION-LOG.md`(위치 항목 갱신)

### ADR-056: 방법론 메타-교훈 — 적대적 재채점·양방향 드리프트·실행 가능 단계 검증·다중 에이전트 영향분석

- **날짜**: 2026-05-31
- **상태**: Accepted
- **맥락**: 제품(kiwoom-rest-trader stock-filter) 회귀 수정 세션(거장 5인×15기준 재채점 → 계층 A 메뉴 jargon 제거·CONFIRM 메뉴행 추가, 계층 B 진위판별)에서 오케스트레이터의 **단독 분석(solo analysis)이 반복적으로 오진**하는 구조적 취약점이 드러났다. 구체적으로 (1) 품질 점수를 적대적 재검증 없이 추정치만으로 종료하려 했고, (2) 단방향(CLAUDE.md→skill) 드리프트 점검이 실재하는 드리프트를 은폐했으며, (3) 사양 산문(spec prose)이 **구현된 적 없는 기계장치(phantom)** — stock-scan→tuning-log의 미구현 cross-write — 를 서술하고 있었고, 오케스트레이터의 5차례 선행 단독 분석이 이를 모두 놓쳤다. 이 실패들은 제품 도메인에서 발생했으나 그 교훈은 부모 게놈에 유전되어야 할 **방법론(framework-level) 차원**이다.
- **결정**: 4개 방법론 메타-교훈을 프레임워크 ADR로 명문화하고, 부모 게놈의 검증·비판 원칙(절대 기준 1·P2·Adversarial Review)에 귀속시킨다:
  1. **An estimate is not a verification** — 품질 점수를 기록하기 **전에 적대적 재채점(adversarial re-score)으로 종료**한다. 추정치는 검증이 아니다. 단독 채점은 자기 확증 편향에 노출되므로, 점수 확정 직전 적대적 페르소나의 재평가를 의무화한다(품질 게이트 L1.5 pACS / Enhanced L2 Adversarial Review의 행동 규범화).
  2. **양방향(bidirectional) 드리프트 검증** — 문서-코드/문서-문서 정합성은 **양방향**으로 점검한다. 단방향(A→B) 점검은 B→A 방향의 실재 드리프트를 은폐한다(이번 세션에서 CLAUDE.md→skill 단방향 점검이 skill 측 드리프트를 놓침). doc-code sync(ADR-048) 점검을 양방향 의무로 격상.
  3. **실행 가능 단계 기준 검증, 산문 기준 금지(phantom detection)** — 행동을 **실행 가능한 단계(executable steps)에 대해** 검증하고, 서술적 산문(descriptive prose)을 근거로 삼지 않는다. 사양 산문은 결코 구현되지 않은 기계장치를 서술할 수 있다(phantom). 이번 세션의 cross-write phantom은 산문에만 존재했고, 유령 산문을 제거한 뒤 단일 기록자(filter-tune를 tuning-log의 TRUE single-writer로) 모델로 backfill·가드를 구현해 해소했다. ADR-053의 "code doesn't lie"를 검증 방법론으로 일반화.
  4. **행동 변경 전 다중 에이전트 적대적 영향분석(multi-agent adversarial impact analysis)** — 동작을 바꾸기 전, 단독 분석이 아닌 다중 에이전트 적대적 영향분석을 수행한다. 이번 세션에서 다중 에이전트 영향분석이 phantom을 적발했고, 오케스트레이터 자신의 선행 단독 분석 5건의 오류를 교정했다. P2(전문성 기반 위임)와 Agent Team·Adversarial Review를 **행동 변경의 사전 게이트**로 확장.
- **근거**:
  - **절대 기준 1(품질) 직접 구현**: 네 교훈 모두 "추정·단방향·산문·단독"이라는 저비용 지름길이 **실증된 오진**을 낳았다는 관찰에서 도출. 품질은 적대적·양방향·실행-기준·다중-에이전트 검증을 통해서만 보장된다.
  - **DNA 유전(soul.md §0)**: 제품 도메인에서 발견되었으나, 검증·비판·투명 게놈에 속하는 일반 원리이므로 자식 시스템에 유전되어야 한다. 따라서 제품 ADR이 아닌 프레임워크 ADR로 기록.
  - **CCP Step 2 강화**: 영향 범위 분석(절대 기준 3)을 단독 수행에서 다중 에이전트 적대적 수행으로 격상하여, 동일 클래스(phantom·단방향 은폐·자기 확증)의 오진 재발을 차단.
- **대안**:
  - 제품 ADR(`docs/architectural-decision-records.md`)에만 기록 → 기각(방법론 차원의 유전 가능 교훈은 부모 게놈에 귀속되어야 함 — soul.md §0).
  - 산문 보강(프롬프트 규칙)만으로 강제 → 기각(이번 세션의 실패 자체가 "산문은 구현을 보장하지 않는다"를 입증 — 메타-교훈 (c)와 모순).
  - 네 교훈을 4개의 개별 ADR로 분리 → 기각(동일 세션·동일 근원(단독 분석의 한계)에서 도출된 단일 메타-교훈 군집이므로 통합이 더 정합적).
- **worked instance(제품 ADR 교차참조)**: 본 메타-교훈의 실증 사례는 제품 ADR-013(Start Routing — 자연어 "시작" → 제품 사용 모드 진입 + Infrastructure Build 노출 차단)과 ADR-014(거장 5인×15기준 재채점 → 계층 A jargon 제거·14/14 매핑·"verbatim" 태그 정정, 계층 B Option B: filter-tune를 tuning-log TRUE single-writer로 + cross-write phantom 제거 + found-but-pending 가드)에 기록되어 있다. 가드레일 테스트 22개(`kiwoom-rest-trader/tests/orchestration/test_tuning_log_single_writer.py`)가 single-writer 불변식을 강제한다. 이 22개는 빌드 pytest L1(119) 및 Safety Hook(131) 테스트 카운트와 별개의 제품 repo 테스트다.
- **관련 ADR**: ADR-048 (doc-code sync — 양방향 격상), ADR-053 ("code doesn't lie" — 실행-기준 검증 일반화); 품질 게이트 L1.5 pACS·Enhanced L2 Adversarial Review·Agent Team 위임(P2)의 행동 규범화. worked instance는 제품 ADR-013/014.
...
| 2026-05-26 | (build) | ADR-052: workflow-executor 스킬 + 13 빌드 에이전트 + 4 게이트 커맨드 |
| 2026-05-26 | (build) | ADR-053: 코드젠 값-동등성 파이프라인 + pytest L1 119 테스트 |
| 2026-05-31 | (build) | ADR-054: validate_state_yaml(차단형) + monitor_translation_output 훅 |
| 2026-05-30 | (build) | ADR-055: 제품 빌드 기준 문서 3종 + 제품 ADR 네임스페이스 분리 |
| 2026-05-31 | (build) | ADR-056: 방법론 메타-교훈 — 적대적 재채점·양방향 드리프트·실행 단계 검증(phantom 적발)·다중 에이전트 영향분석 (worked instance: 제품 ADR-013/014) |
...
- **위치**: 프로젝트 루트 (`DECISION-LOG.md`) — 프레임워크 ADR. 제품 빌드 ADR은 `docs/architectural-decision-records.md`(ADR-001~014, 독립 번호 체계)에 별도로 존재하며 본 로그와 네임스페이스가 다르다.
