...
│   │   └── (+13 빌드 전용 — param-extractor·pipeline-analyzer·error-analyzer·research-integrator·architect·{claude-md,scan,tune}-{designer,builder}·infra-validator·smoke-tester; 상세 prompt/workflow.md)
│   ├── commands/                    ← Slash Command 10개
│   │   ├── install.md               (/install — Setup Init 검증)
│   │   ├── maintenance.md           (/maintenance — 건강 검진)
│   │   └── (+8 — accept-system·review-design·review-research·review-translation 휴먼 게이트 + run·resume·setup·verify-prompts 프롬프트 러너)
...
│   │   ├── validate_state_yaml.py   (state.yaml SOT 무결성 차단 가드 — PostToolUse, exit 2)
│   │   ├── monitor_translation_output.py (.ko.md 번역 품질 모니터 — PostToolUse, 비차단)
...
│       ├── workflow-executor/       (워크플로우 실행 오케스트레이터 — 12단계 빌드 구동, SOT 단독 기록)
│       └── doctoral-writing/        (박사급 학술 글쓰기)
├── translations/glossary.yaml       ← 번역 용어 사전 (466개 용어)
├── prompt/                          ← 워크플로우 실행 인스턴스 (workflow.md 12단계 스펙 + .claude/state.yaml SOT + codegen 값-동등성 게이트 + pytest L1 + 12단계 산출물)
├── prompt-runner/                   ← 무인 실행 하니스 (110 프롬프트/35 세션, run.py)
├── tests/                           ← 최상위 pytest 스위트 (codegen·SOT 스키마·상태머신 불변식, 64 테스트)
...
| PostToolUse (Write\|Edit) | `validate_state_yaml.py` | `state.yaml` SOT 스키마 검증 — 무효 쓰기 차단 (exit 2, 유일한 차단형 PostToolUse 훅) |
| PostToolUse (Write) | `monitor_translation_output.py` | `.ko.md` 번역 품질 모니터 (비차단, exit 0) |
...
| "워크플로우 실행", 실행 세션 시작·재개 | `workflow-executor` | SKILL.md (state.yaml 우선 읽기) |
