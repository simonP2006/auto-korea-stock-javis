...
### 결정적 필드 vs LLM 필드 분리 (P1 Code Analysis 특화)

코드 분석 워크플로우에서 에이전트가 소스 코드를 직접 해석하면 구조적 데이터(상수, 타입, 호출 체인)의 **누락·오독·날조** 위험이 높다. Python AST 전처리로 결정적 필드를 추출하고, LLM에는 의미 해석 필드만 위임하여 할루시네이션 표면적을 최소화한다.

**필드 분리 원칙:**

| 필드 유형 | 추출 주체 | 예시 | 정확도 |
|----------|---------|------|--------|
| `deterministic` | Python (AST/regex) | 변수명, 타입 어노테이션, 리터럴 값, 파일 경로, 행 번호, 호출 관계 | 100% |
| `llm` | 에이전트 | 의미 해석, 설계 의도, 트리거 조건 서술, 한국어 메시지 | 가변 |

**워크플로우 설계 적용:**

```markdown
### N. (team) 코드 분석
- **Phase 0 Pre-processing** (결정적 — Bash/Python):
  - `codegen/extract_params.py` — AST 기반 상수·타입·값 추출
  - `codegen/trace_calls.py` — 호출 체인 DAG 구성
  - `codegen/collect_errors.py` — 예외 클래스·exit code 수집
- **Phase 1 Agent** (LLM 해석):
  - `@param-analyzer`: Phase 0 출력의 `meaning` 필드만 작성
  - `@call-interpreter`: DAG 기반 다이어그램 + 아키텍처 해석
  - `@error-documenter`: 트리거 조건 서술 + 한국어 메시지 작성
- **Agent 제약**: deterministic 필드 수정 금지 — 에이전트는 llm 필드만 채움
```
...
| 작업 | AST 전처리 | 에이전트 |
|------|-----------|---------|
| 상수·변수·타입 인벤토리 | `AnnAssign`, `Assign` 노드 추출 | 의미 해석만 |
| 함수 호출 체인 추적 | `Call`, `Import` 노드 → DAG | 다이어그램·해석만 |
| 예외·에러 패턴 수집 | `Raise`, `ExceptHandler` 노드 | 트리거·메시지만 |
| 설계 의도·비즈니스 로직 해석 | 해당 없음 | 에이전트 전담 |
| 코드 리뷰·품질 평가 | 해당 없음 | 에이전트 전담 |
