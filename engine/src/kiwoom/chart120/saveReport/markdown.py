"""120분봉 + 120분봉 MA 결과를 마크다운 리포트로 저장.

저장 경로 규칙: ``reports/<실행일자 YYYYMMDD>/<종목명(코드)>/chart120.md``.
...
_FILENAME: Final[str] = "chart120.md"
...
def render_markdown(
    ...
    *,
    ...
    Args:
        df: ``get_120min_with_ma()`` 가 반환한 DataFrame.
        stk_cd: 종목코드.
    ...
    if df.empty:
        return f"# {title_name} 120분봉 분석\n\n응답 데이터 없음.\n"

    last = df.iloc[-1]
    first = df.iloc[0]

    lines: list[str] = [
        f"# {title_name} 120분 차트 + 120분봉 MA 분석",
        "",
        "## 기본 정보",
    ...
    lines += [
        ...
        "- **MA 산출 기준**: 120분봉 종가 기준 단순이동평균(SMA)",
        "",
        "## 최근 봉 스냅샷",
    ...
    lines += [
        "",
        "## 비고",
        "- 가격 단위: 원, 거래량 단위: 주(120분봉별 체결량)",
        "- MA 값은 120분봉 종가의 N 기간 단순이동평균(SMA)",
        "- 데이터 출처: 키움 OpenAPI ka10080(주식분봉차트조회, tic_scope=120)",
...
def save_chart120_markdown(
    ...
    *,
    ...
    """마크다운 렌더 후 ``<output_root>/<오늘 YYYYMMDD>/<종목명(코드)>/chart120.md`` 에 저장.
