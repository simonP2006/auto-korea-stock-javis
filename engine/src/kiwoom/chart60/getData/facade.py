"""60분봉 + 60분봉 종가 기준 MA 퍼사드.

흐름:
    1. ``KiwoomAuth`` 토큰 확보 (``ChartService`` 내부에서 자동).
    2. ``ka10080`` 60분봉을 ``max(ma_windows) + bars + 마진`` 만큼 누적 조회.
    3. 60분봉 종가 시리즈로 MA10/20/60/306 등 SMA 계산 (HTS [0600] 키움종합챠트
       60분봉 화면의 MA 와 동일).
    4. 가장 최근 ``bars`` 행을 시간 오름차순 DataFrame 으로 반환.

이전 버전(2026년 5월 이전)은 일봉 종가 기준 MA 를 60분봉 행에 left-join 하여
"같은 거래일의 모든 60분봉이 동일한 MA 값을 갖는" 구조였으나, HTS 60분봉 차트
의 MA 와 의미가 달랐다 (예: 005930 5/8 19:00봉 일봉MA10=238,650 vs HTS
60분봉MA10=270,300). 본 모듈은 HTS 와 일치시키기 위해 60분봉 기준 MA 로 전환됐다.
"""
...
async def get_60min_with_ma(
    ...
    *,
    ...
    """특정 종목의 60분봉 + 60분봉 종가 기준 MA 결합 결과를 반환한다.

    ``days`` 또는 ``bars`` 중 하나로 결과 범위를 지정한다. 둘 다 ``None`` 이면
    ``bars=16`` 이 적용되어 가장 최근 60분봉 16개를 반환한다. 둘 다 지정하면
    ``ValueError`` 가 발생한다.

    MA 는 60분봉 종가의 N 기간 단순이동평균(SMA) 으로, HTS [0600] 키움종합챠트
    60분봉 화면의 MA 와 일치한다 (예: 5/8 19:00봉 ma10 = 직전 10개 60분봉 종가
    평균 = 270,300).
    ...
    Args:
        ...
        bars: 결과에 포함할 최근 60분봉 행 수(기본 모드).
        ma_windows: 60분봉 종가 기준 MA 윈도우 튜플(기본 ``(10,20,60,306)``).
    ...
    if bars is not None:
        ...
        # days 모드는 거래일 단위라 60분봉 단위로 환산할 때 정확하지 않으므로
        # 풍부하게 잡는다.
        assert days is not None
        target_bars = days * 16  # NXT 포함 1거래일 약 12~16봉
    fetch_bars = max(ma_windows) + target_bars + _HISTORY_MARGIN

    minute_rows = await chart.fetch_minute_60(
    ...
    if minute_df.empty:
        logger.warning("60분봉 응답 없음 stk_cd={s}", s=stk_cd)
...
# 하위 호환 — 이전 이름 유지 (deprecated). 신규 코드는 get_60min_with_ma 사용.
get_60min_with_daily_ma = get_60min_with_ma
