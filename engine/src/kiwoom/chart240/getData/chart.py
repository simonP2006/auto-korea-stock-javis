...
from src.kiwoom.chart240.getData.models import DailyCandle, MinuteCandle
...
_BARS_PER_DAY_240M: Final[int] = 2  # 정규장 6.5h 기준 240분봉 ≈ 1.6봉, 여유 포함
_DAILY_MIN_ROWS_DEFAULT: Final[int] = 330  # MA306 산출 + 휴장일 마진
_MINUTE_BARS_DEFAULT: Final[int] = 16  # 가장 최근 240분봉 행 수 기본 (= 8거래일 × 2봉)
...
class ChartService:
    ...
    async def fetch_minute_240(
        ...
        *,
        ...
        """ka10080 240분봉(4시간봉)을 누적 조회.

        ``days`` 또는 ``bars`` 중 하나로 종료 조건을 지정한다. 둘 다 ``None``
        이면 ``bars=16`` (가장 최근 240분봉 16개 = 8거래일분)이 적용된다.
        ...
        if bars is not None:
            ...
            target_rows = days * _BARS_PER_DAY_240M
