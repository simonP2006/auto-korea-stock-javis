"""240분봉 + 240분봉 종가 기준 MA 퍼사드.

흐름:
    1. ``KiwoomAuth`` 토큰 확보 (``ChartService`` 내부에서 자동).
    2. ``ka10080`` 240분봉(4시간봉)을 ``max(ma_windows) + bars + 마진`` 만큼
       누적 조회.
    3. 240분봉 종가 시리즈로 MA10/20/60/306 등 SMA 계산.
...
async def get_240min_with_ma(
    ...
    *,
    ...
    """특정 종목의 240분봉 + 240분봉 종가 기준 MA 결합 결과를 반환한다.

    ``days`` 또는 ``bars`` 중 하나로 결과 범위를 지정한다. 둘 다 ``None`` 이면
    ``bars=16`` 이 적용되어 가장 최근 240분봉 16개(= 8거래일분)를 반환한다.
    둘 다 지정하면 ``ValueError`` 가 발생한다.

    MA 는 240분봉 종가의 N 기간 단순이동평균(SMA) 이다 (예: ma10 = 직전
    10개 240분봉 종가의 평균).
    ...
    Args:
        ...
        bars: 결과에 포함할 최근 240분봉 행 수(기본 모드).
        ma_windows: 240분봉 종가 기준 MA 윈도우 튜플(기본 ``(10,20,60,306)``).
    ...
    if bars is not None:
        ...
        # days 모드는 거래일 단위라 240분봉 단위로 환산할 때 정확하지 않으므로
        # 풍부하게 잡는다.
        assert days is not None
        target_bars = days * 16  # NXT 포함 1거래일 약 1~2봉이지만 여유 확보
    fetch_bars = max(ma_windows) + target_bars + _HISTORY_MARGIN

    minute_rows = await chart.fetch_minute_240(
    ...
    if minute_df.empty:
        logger.warning("240분봉 응답 없음 stk_cd={s}", s=stk_cd)
...
# 하위 호환 — 이전 이름 유지 (deprecated). 신규 코드는 get_240min_with_ma 사용.
get_240min_with_daily_ma = get_240min_with_ma
