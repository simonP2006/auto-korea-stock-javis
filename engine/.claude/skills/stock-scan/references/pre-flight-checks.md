...
- **Bash**: `test -d /Users/tajun/spJavis/kiwoom-rest-trader`
- **기대 exit**: `0`
...
- **Bash**: `[ -x /Users/tajun/spJavis/kiwoom-rest-trader/.venv/bin/python ] && /Users/tajun/spJavis/kiwoom-rest-trader/.venv/bin/python --version`
...
- **권한 probe 실패 (R-11)**: 첫 `cd ${KRT_ROOT} && .venv/bin/python --version` 호출이 permission-denied로 거부되면 사용자에게 `"Bash(cd /Users/tajun/spJavis/kiwoom-rest-trader && *)"`를 `.claude/settings.local.json`의 allow 목록에 추가하라고 안내하거나 `/install` slash command 사용을 제안.
...
- **Bash**: `test -w /Users/tajun/spJavis/kiwoom-rest-trader/reports`
- **기대 exit**: `0`
...
- **Bash**:
  ```
  python3 -c "import json,sys; p='/Users/tajun/spJavis/kiwoom-rest-trader/reports/{YYYYMMDD}/prefetchManifest.json'; d=json.load(open(p)); errs=sum(1 for s in d['by_stock'].values() for v in s.values() if v not in ('ok','empty','null',None,'')); print(f'OK_total={len(d[\"by_stock\"])} ERR={errs}'); sys.exit(0 if errs==0 else 1)"
  ```
...
- **Bash (filter-tune이 실행)**: `grep -n '\b{VARIABLE_NAME}\b' /Users/tajun/spJavis/kiwoom-rest-trader/{file_path}`
...
- **Bash**: `test -d /Users/tajun/spJavis/kiwoom-rest-trader/reports/filter-tune.lock`
